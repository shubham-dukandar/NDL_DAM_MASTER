...
namespace Global_Master_Helper.Process.DealNote
{
    public class DealNoteApproval
    {
        public static List<ReqDetailForDealNote> getData(int ProcessID, int InstanceID, string MasterUrl, string WfeUrl)
        {
            ...
            try
            {
                ...
                var finalpoData = (from hdr in context.T_DEALNOTE_HDRs
                                   ...
                                   //join me in context.M_ACCESS_USR_ROLE_MAPs on hdr.CREATED_BY equals me.USER_ADID //&& me.ACCESS_ROLE_ID==3
                                   ...
                                   join re in context.M_REGIONs on hdr.FK_REGION_ID equals re.PK_REGION_ID
                                   join div in context.M_DIVISIONs on hdr.FK_DIVISION_ID equals div.PK_DIVISION_ID
                ...
                for (int i = 0; i < finalpoData.Count; i++)
                {
                    ...
                    obj.Business_Justification = finalpoData[i].Business_Justification;
                    obj.Recommendation = finalpoData[i].Recommendation;
                    obj.Imp_Timelines = finalpoData[i].Imp_Timelines;

                    obj.FK_REGION_ID = Convert.ToInt32(finalpoData[i].FK_REGION_ID);
                    obj.REGION_NAME = (finalpoData[i].REGION_NAME);
                    obj.FK_DIVISION_ID = Convert.ToInt32(finalpoData[i].FK_DIVISION_ID);
                    obj.DIVISION_NAME = (finalpoData[i].DIVISION_NAME);
        ...
        public static List<CurrentApprovalMatrix> getCurrentApprovalMatrixData(string SesssionID, string Flag, int requestforid, string status)
        {
            ...
            try
            {
                GlobleMasterDataContext context = new GlobleMasterDataContext();
                string rolename = "";
                if (status == "Pending With HOD Approver")
                {
                    rolename = "HOD  Approver";
                }
                if (status == "Pending With Finance Team")
                {
                    rolename = "Finance Team";
                }
                if (status == "Pending With CFO Approver")
                {
                    rolename = "CFO Approver";
                }
                if (status == "Pending With CEO  Approver")
                {
                    rolename = "CEO  Approver";
                }
                ...
                if (rolename != "")
                {
                    var finalpoData = (
                       ...
                       where uam.USER_ADID == SesssionID && am.FLAG == Flag && am.ROLE_NAME == rolename
                       select new
                       {
                           am.ROLE_NAME,
                           am.ORDER_ID
                ...
                //    join ac in context.M_ACCESS_ROLEs on am.ROLE_NAME equals ac.ROLE_NAME
                //    join uam in context.M_ACCESS_USR_ROLE_MAPs on Convert.ToInt32(ac.PK_ROLE_ID) equals Convert.ToInt32(uam.ACCESS_ROLE_ID)
                //    where uam.USER_ADID == SesssionID && am.FLAG == Flag //&& am.ROLE_NAME == "Finance Team"
                //    select new
                //    {
                //        am.ROLE_NAME,
                //        am.ORDER_ID
        ...
        public static List<NextApprovalMatrix> getNextApprovalMatrix(int orderid, string Flag, string rolename)
        {
            ...
            try
            {
                ...
                var finalpoData = (
                    ...
                    join ac in context.M_ACCESS_ROLEs on am.ROLE_NAME equals ac.ROLE_NAME
                    join uam in context.M_ACCESS_USR_ROLE_MAPs on Convert.ToInt32(ac.PK_ROLE_ID) equals Convert.ToInt32(uam.ACCESS_ROLE_ID)
                    where am.ORDER_ID == (orderid + 1) && am.FLAG == Flag && am.ROLE_NAME != rolename
                    select new
                    {
                        am.ROLE_NAME,
                        am.ORDER_ID
        ...
        public static string SendBackReq(int processId, int instanceId, string stepName, string action, string requester_Mail, string[] Dval, string hdrStatus, string requestNo, string nextAppEmailID, string MasterUrl, string WfeUrl, string portalurl, string currentapprvremailid, string REMARK, string initiatorid, string EMP_NAME, string requestperformer)
        {
            string Result = "";
            ...
            using (GlobleMasterDataContext context = new GlobleMasterDataContext())
            {
                try
                {
                    ...
                    else if (action == "SENDBACKCLARIFICATION")
                    {
                        hdrobj.REMARK = REMARK;
                        hdrobj.STATUS = hdrStatus;
    ...
    public class ReqDetailForDealNote
    {
    ...
    public class CurrentApprovalMatrix
    {
        public string portalurl { get; set; }
        public string Master_API { get; set; }
    ...
    public class NextApprovalMatrix
    {
        public string portalurl { get; set; }
        public string Master_API { get; set; }
        public string Flag { get; set; }
